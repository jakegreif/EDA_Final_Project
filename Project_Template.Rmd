...
westfork.flood.freq$site_no <- c(08057000)
westfork.drought.freq$site_no <- c(08057000)
...
```{r}
## Exploratory Graphs
explore1 <- ggplot(NULL, aes(x = datetime, y = discharge.mean)) + 
      geom_point(data = trinity.flow,
                 color = "red", alpha = 0.5) +
      geom_point(data = westfork.flow,
                 color = "blue", alpha = 0.5)

explore2 <- ggplot(NULL, aes(x = datetime, y = discharge.mean)) + 
      geom_smooth(data = trinity.flow,
                 color = "red") +
      geom_smooth(data = westfork.flow,
                 color = "blue")

explore3 <- ggplot(NULL, aes(x = discharge.mean)) +
  geom_histogram(data = trinity.flow, fill = "red") +
  geom_histogram(data = westfork.flow, fill = "blue", alpha = 0.5) +
  ylim(0,1500) +
  xlim(0,20000)
 
explore4 <- ggplot(all.flow.data) +
...
ggplot(trinity.flood.freq) +
  geom_point(aes(x = Year, y = freq))
ggplot(trinity.drought.freq) +
  geom_point(aes(x = Year, y = freq))
ggplot(westfork.flood.freq) +
  geom_point(aes(x = Year, y = freq))
ggplot(westfork.drought.freq) +
  geom_point(aes(x = Year, y = freq))
```
...
trinity.flood.trend <- lm(trinity.flood.freq$freq ~ trinity.flood.freq$Year)
trinity.drought.trend <- lm(trinity.drought.freq$freq ~ trinity.drought.freq$Year)
westfork.flood.trend <- lm(westfork.flood.freq$freq ~ westfork.flood.freq$Year)
westfork.drought.trend <- lm(westfork.drought.freq$freq ~ westfork.drought.freq$Year)

summary(trinity.flood.trend)
summary(trinity.drought.trend)
summary(westfork.flood.trend)
summary(westfork.drought.trend)
